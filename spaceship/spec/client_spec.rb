...
describe Spaceship::Client do
  ...
  describe 'retry' do
    {
      Faraday::Error::TimeoutError => Faraday::Error::TimeoutError,
      Faraday::Error::ConnectionFailed => Faraday::Error::ConnectionFailed,
      Errno::EPIPE => Faraday::Error::ConnectionFailed
    }.each do |thrown, caught|
      it "re-raises #{caught} error when retry limit reached" do
        stub_client_request(thrown, 6, 200, nil)

        expect do
          subject.req_home
        end.to raise_error(caught)
      end
